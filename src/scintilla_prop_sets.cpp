...
void cfg_sci_prop_sets::import_from_file(const char* filename)
{
	...
	helpers::read_file_wide(pfc::stringcvt::string_wide_from_utf8_fast(filename), content);
	pfc::string8_fast text = pfc::stringcvt::string_utf8_from_wide(content.get_ptr());

	typedef pfc::chain_list_v2_t<pfc::string8_fast> t_string_chain_list;
	t_string_chain_list lines;
	pfc::splitStringByLines(lines, text);
